...
int FFMPEG_Info::get_MediaInfo(const char *filePath, QStringList &infoList, STmedia *media_info)
{
    int ret = SUCCESS;
    media_info->video_codec = AV_CODEC_ID_NONE;
    media_info->audio_codec = AV_CODEC_ID_NONE;
    char strtmp[STR_LEN];
    ...
    if(video_stream_idx >= 0){
        ...
        media_info->video_codec = pVideoStream->codec->codec_id;
        infoList <<strtmp;
    ...
    if(audio_stream_idx >= 0){
        ...
        media_info->audio_codec = pAudioStream->codec->codec_id;
        infoList <<strtmp;
